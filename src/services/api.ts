...
// Helper function to check if a code is newly added based on added_by field
const isNewlyAddedCode = (code: any): boolean => {
  // Check if the code has an added_by field with value "admin"
...
export const fetchCodingResults = async (docId?: string) => {
  try {
    ...
    if (data && data.results) {
      ...
      if (Array.isArray(results.primary_codes)) {
        const primaryCodes = results.primary_codes.map((code) => ({
          ...
          // Check if this is a newly added code
    ...
    console.log(
      ...
      codesArray.filter((c) => c.is_newly_added).length
    );
    console.log(
      "Newly added codes:",
      codesArray.filter((c) => c.is_newly_added).map((c) => c.diagnosis_code)
    );
